...
CHECK $? "downloading user service from s3 bucket.."
...
CHECK $? "unzipping user service in app directory.."

npm install &>>$LOGFILE

CHECK $? "installing dependencies for user service in app directory.."

cp /home/centos/automationscripts/user.service /etc/systemd/system/user.service &>>$LOGFILE

CHECK $? "adding user service to daemon service"
...
CHECK $? "copying mongo repo.. to yum.repos.d"
